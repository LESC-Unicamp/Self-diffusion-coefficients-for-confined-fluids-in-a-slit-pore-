...
/////////////// 9) Steps of time of holding                    /////////  
/////////////// Spera & Braga - October, 2021 //////////////////////////
...
/////////////// MSD from Equation 14 from Liu et al. 2004 //////////////
	...
	int hold = atof(argv[9]); //time tolerance
		...
		for(t=j;t<tmax;t++){
			...
			for(i=0;i<aux;i++){ 					 
				if(matz[t][i]>=hmin_par && matz[t][i]<=hmax_par && tag[j][i]!='\0'){
					...
					if(hold !='\0' && t<tmax-hold && matz[t+hold][i]>=hmin_par && matz[t+hold][i]<=hmax_par && tag[j][i]!='\0') {
						sum_x[t] = sum_x[t]+(matx[t][i]-matx[j][i])*(matx[t][i]-matx[j][i]);
                                                sum_y[t] = sum_y[t]+(maty[t][i]-maty[j][i])*(maty[t][i]-maty[j][i]);
						n[t]=n[t]+1;
					}
...
/////////////// Eq. 27 Franco et al. 2016 //////////////////////////////
	...
	fprintf(in9,"%f %f %f\n",(hmax_perp+hmin_perp)*0.5,(1.0/alpha1),tau);
